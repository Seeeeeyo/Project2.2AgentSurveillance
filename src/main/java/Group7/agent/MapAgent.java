    ...
    public void updateMap(ArrayList<ObjectPercept> objectPerceptArrayList){
            ...
            for (int i = 0;i<objectPerceptArrayList.size();i++){
                ...
                switch (type) {
                    ...
                    case TargetArea: if (debug) System.out.println("Update point successfully: target Place");map[x][y] = targetPlace; // array out of bounds sometimes
                    break;
...
class TEST{
    ...
    public static void main(String[] args) {
        MapAgent ma = new MapAgent(10,10);

//        ma.actionHistory.add(new ActionHistory(2,0));
//        ma.updateCurrentAngle();
//        ma.actionHistory.add(new ActionHistory(2,0));
//        ma.updateCurrentAngle();

//        ma.printCurrentAngle();

        ma.actionHistory.add(new ActionHistory(2,350));
        ma.updateCurrentAngle();

        ma.actionHistory.add(new ActionHistory(1,2));
        ma.updateCurrentPosition();
        ma.printCurrentPosition();

        ma.actionHistory.add(new ActionHistory(2,45));
        ma.updateCurrentAngle();
        ma.printCurrentAngle();
        ma.actionHistory.add(new ActionHistory(1,2));
        ma.updateCurrentPosition();
